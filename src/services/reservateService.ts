...
export const reservateService = {
  getAllReservates: async (): Promise<Reservate[]> => {
    ...
    console.log(
      "🌐 [RESERVATE SERVICE] URL completa:",
    ...
    try {
      ...
      console.log(
        "🔢 [RESERVATE SERVICE] Cantidad de reservas:",
  ...
  updateReservate: async (
    code: string,
    reservateData: Partial<ReservateData>
  ): Promise<Reservate> => {
    ...
    console.log(
      "🌐 [RESERVATE SERVICE] URL completa:",
      `http://localhost:4000/api/v1${API_BASE_URL}/${code}/update`
    );
    ...
    try {
      ...
      console.log(
        "✅ [RESERVATE SERVICE] Actualización exitosa:",
        response.status
      );
      console.log(
        "📊 [RESERVATE SERVICE] Respuesta del servidor:",
        response.data
      );
      return response.data;
    } catch (error) {
      ...
      if (error && typeof error === "object" && "response" in error) {
        ...
        console.error(
          "❌ [RESERVATE SERVICE] Error response:",
          axiosError.response?.data
        );
        console.error(
          "❌ [RESERVATE SERVICE] Error status:",
  ...
  deleteReservate: async (code: string): Promise<void> => {
    ...
    console.log(
      "🌐 [RESERVATE SERVICE] URL:",
      `${API_BASE_URL}/${code}/delete`
    );
    try {
      const response = await apiClient.delete(`${API_BASE_URL}/${code}/delete`);
      console.log(
        "✅ [RESERVATE SERVICE] Reserva eliminada exitosamente:",
        response.status
      );
    } catch (error) {
